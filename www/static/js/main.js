...
$(document).ready(function() {
  ...
  ws.onopen = function() {
    ws.send(["load_user", "53d2452ee2a5fc1e2ea033ab", "53d2452ee2a5fc1e2ea033ab"]);
  ...
  var updateUserList = function() {
    ...
    _.each(all_users, function(user) {
      ...
      if (window.default_user != user._id.$oid) {
        $('<li class="user_list_item" data-user-id="' + user._id.$oid + '"><a href="#" class="' + current_user_class + '">' + user.name + '</a></li>').insertBefore($('#user_dropdown ul #new_user_list_item'));
      } else {
        $('<li class="user_list_item" data-user-id="' + user._id.$oid + '"><a href="#" class="' + current_user_class + '">' + user.name + '</a></li>').prependTo($('#user_dropdown ul'));
      }
  ...
  ws.onmessage = function(evt) {
    ...
    if (d[0] == "init") {
      ...
      updateUserList();
      gestureEditor = new BezierCurveEditor("gesture-editor", curve_channels, sh, [600, 500]);
      ...
      $('.ems_on_off').on("click", function(event) {
        ...
        $(this).parent().toggleClass('active').toggleClass('btn-success').toggleClass('btn-primary');
        if ($(this).is(':checked')) {
      ...
      //file://localhost/Users/patrikjonell/Dropbox/hpi/New%20Unity%20Project/test/test.html
      ...
      $(".slider.ems_value").slider({
        ...
        slide: function(event, ui) {
          ...
          if (ui.value < channel_data[id]["min"] || ui.value > channel_data[id]["max"]) {
            return false;
          } else {
            channel_data[id]["value"] = ui.value
            $base.find(".ems_value_span").html(channel_data[id]["value"]);
            ws.send(["calibrate", "ems_value", id, channel_data[id]["value"]]);
          }
        }
      });
      ...
      $(".slider.ems_limit").slider({
        ...
        change: function(event, ui) {
          var $base = $(event.target).parents(".channel_wrapper");
          var id = $base.data("channel-id");
          ws.send(["save_channels", window.current_user_id, id, ui.values[0], ui.values[1]]);
        ...
        slide: function(event, ui) {
          ...
          if ($base.find(".slider.ems_value").slider("value") < channel_data[id]["min"]) {
            $base.find(".slider.ems_value").slider("value", channel_data[id]["min"]);
            channel_data[id]["value"] = channel_data[id]["min"];
            //$base.find(".ems_value_span").html(channel_data[id]["min"]);

          }
          if ($base.find(".slider.ems_value").slider("value") > channel_data[id]["max"]) {
            $base.find(".slider.ems_value").slider("value", channel_data[id]["max"]);
            channel_data[id]["value"] = channel_data[id]["max"];
            //$base.find(".ems_value_span").html(channel_data[id]["max"]);
          }

          ws.send(["calibrate", "ems_min_max", id, channel_data[id]["min"], channel_data[id]["max"]]);
          ws.send(["calibrate", "ems_value", id, channel_data[id]["value"]]);
    ...
    if (d[0] == "newly_added_user_id") {
      ...
      _.each(channel_data, function(channel, channel_id) {
        ws.send(["save_channels", user._id.$oid, id, channel['min'], channel['max']]);
      });
    ...
    if (d[0] == "user_info") {
      ...
      _.each(user.channels, function(channel, channel_name) {
        ...
        $("#" + channel_name + "_calib .slider.ems_limit").slider("values", [channel['min'], channel['max']]);
        $("#" + channel_name + "_calib .slider.ems_value").slider("value", channel['min']);
      });
    ...
    if (d[0] == "colliders_init") {
      ws.send(["colliders_init_cb", (window.current_user_id == -1 ? window.default_user : window.current_user_id)]);


    }

    if (d[0] == "colliders") {
      var gesture_data = JSON.parse(d[1].replace(/§/g, ', ').replace(/\'/g, '"'));
      var envelope_data = JSON.parse(d[2].replace(/§/g, ', ').replace(/\'/g, '"'));
      ...
      for (each in envelope_data) {
        ...
        envelopEditors[envelope_id].showEnvelope(envelope_id, gesture, duration, individualPoints);
